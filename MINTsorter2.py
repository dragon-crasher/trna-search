...
def process_files(folder_path, name="selected"):
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.txt') and 'exclusive' in filename:  # Remove 'expression' filter if not needed
            ...
            if r1_match:
                new_filename = f"{accession_code}_r1.txt"
            elif r2_match:
                new_filename = f"{accession_code}_r2.txt"
            else:
                new_filename = f"{accession_code}.txt"
...
if len(sys.argv) != 2:
    ...
    sys.exit(1)
name = sys.argv[1]
process_files("/raid/anirudh/bioinformatics/MINT/outputs/", name)
